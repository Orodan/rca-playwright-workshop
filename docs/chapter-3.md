...
::: tip INFO
ℹ️ Si vous voulez ne lancer qu'un test en particulier, remplacez `test` par `test.only`.
:::
...
- Dans chacun des 2 tests restants, naviguez vers `https://playwright.dev/`
...
npm run test:e2e
```
- Vérifiez que vos screenshots ont été correctement générés.
- Relancer vos tests et vérifiez qu'ils passent au vert cette fois.
